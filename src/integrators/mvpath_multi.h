...
MI_VARIANT typename MVPT::Mask MVPT::sample_multi(const Scene *scene, const MultiSensor<Float, Spectrum> *sensor,
    ...
    auto [p_bsdf_val, p_bsdf_pdf, p_bsdf_sample, p_bsdf_weight] =
        bsdf->eval_pdf_sample(bsdf_ctx, si, wo, rand_1, rand_2);


    BSDFData bsdf_data = dr::zeros<BSDFData>();
    // eval_rougness() is extension to stock BSDF class
    bsdf_data.alpha = bsdf->eval_rougness(si, active);
    bsdf_data.sqr_a = dr::fmsub(bsdf_data.alpha, bsdf_data.alpha, 1.f);
    bsdf_data.rsqrt_a = dr::rsqrt(bsdf_data.alpha);

    Float bsdf_pdf           = p_bsdf_pdf;
    BSDFSample3f bsdf_sample = p_bsdf_sample;

    Bool not_delta = !has_flag(p_bsdf_sample.sampled_type, BSDFFlags::Delta) &&
                     !has_flag(p_bsdf_sample.sampled_type, BSDFFlags::Null);
    Bool delta   = !not_delta;
    Bool reuse   = !has_flag(p_bsdf_sample.sampled_type, BSDFFlags::Empty) && p_hit && bsdf_smooth && not_delta;
    Bool diffuse = has_flag(p_bsdf_sample.sampled_type, BSDFFlags::Diffuse);
    ...
    if (should_reuse) {
        ...
        for (uint32_t k = 1; k < n_sensors; k++) {
            ...
            if (m_sa_mis) {
                ...
                dr::masked(sample_k.pdfM, diffuse) = 1.f; 
                Float pdf_Mat =
                    m_fast_mis
                ...
                // Float pdf_Mat = dr::select(diffuse, 1.f, tv);
                Float J     = Jp / p_Jp;
                ...
                valid &= sampler->next_1d(valid) < pdf_Sel;
                ...
                bsdf_pdf += dr::select(direct, bsdf_pdf_k, 0.f);
    ...
    if (sensor_mis) {
        for (uint32_t k = 0; k < n_sensors; k++) {
            ...
            if (k > 0)
                pdfSum += sample_k.pdf;
            Bool cond = k > 0 ? sample_k.valid : reuse;
            // Use dr::is_stmt to potentially skip the inner loop.
            pdfSum += DR_IF(
                cond, (m_fast_mis, pdfMk, inv_Jk, si_k, samples, n_sensors, k, bsdf_data),
                (Float pdfSum = 0.f;
                 ...
                 for (uint32_t j = 1; j < n_sensors; j++) {
                     ...
                     Float pdf_Mat =
                         m_fast_mis
                             ? tv_pdf_fast(sample_j.wo_r, si_k.wi, pdfMk, bsdf_data, valid)
    ...
    // Primary not delta indirect mask
    Mask p_not_delta = not_delta && p_sample.indirect;
    ...
    if (dr::any_or<true>(active_em)) {
        ...
        if (sensor_mis) {
            ...
            p_bsdf_val     = si.to_world_mueller(p_bsdf_val, -wo, si.wi);
            uint32_t total = should_reuse ? n_sensors : 1;
            for (uint32_t k = 0; k < total; k++) {
                samples[k].result[active_em] = spec_fma(p_bsdf_val, emis_mis, samples[k].result);
                samples[k].weight            = 1;
            }
        }
    }
    ...
    if (sensor_mis) {
        ...
        for (uint32_t k = 0; k < n_sensors; k++) {
            ...
            Spectrum bsdf_val_k = 0;
            Float bsdf_pdf_k    = 0;
            // Special case for primary camera
            if (k == 0) {
                if (dr::any_or<true>(p_not_delta)) {
                    auto [bsdf_val_t, bsdf_pdf_t] = bsdf->eval_pdf(bsdf_ctx, si_k, bsdf_sample.wo, valid);
                    // Weight is bsdf_val / bsdf_sample.pdf for delta surfaces
                    bsdf_val_k = dr::select(p_not_delta, bsdf_val_t, p_bsdf_weight);
                    bsdf_pdf_k = dr::select(p_not_delta, bsdf_pdf_t, p_bsdf_sample.pdf);
                }
                valid &= (bsdf_pdf_k > 0.f || delta);
            }
            // Evaluate secondary cameras
            else if (dr::any_or<true>(valid)) {
                std::tie(bsdf_val_k, bsdf_pdf_k) = bsdf->eval_pdf(bsdf_ctx, si_k, bsdf_sample.wo, valid);
                valid &= bsdf_pdf_k > 0.f;
            }
            ...
            if (dr::any_or<true>(valid)) {
                ...
                bsdf_sample.pdf += bsdf_pdf_k;
                n_indir += Float(valid);
            }
            sample_k.indirect &= valid;
        }
        // Mixture pdf -> average when not delta, otherwise primary delta pdf
        bsdf_sample.pdf = dr::select(p_not_delta, bsdf_sample.pdf / n_indir, p_bsdf_sample.pdf);
    } // Normal BSDF case
    else {
        p_bsdf_weight = si.to_world_mueller(p_bsdf_weight, -p_bsdf_sample.wo, si.wi);
        if (dr::grad_enabled(pd.ray)) {
            ...
            p_bsdf_weight[bsdf_pdf_2 > 0.f] = bsdf_val_2 / dr::detach(bsdf_pdf_2);
        }
    }
    // ------ Update loop variables based on current interaction
    // ------

    pd.throughput = sensor_mis ? 1.f : p_bsdf_weight;
