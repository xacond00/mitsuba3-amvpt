...
template <typename Float, typename Spectrum> void scene_static_accel_initialization() {
    Scene<Float, Spectrum>::static_accel_initialization();
}

template <typename Float, typename Spectrum> void scene_static_accel_shutdown() {
    Scene<Float, Spectrum>::static_accel_shutdown();
}

template <typename Float, typename Spectrum>
void render_internal(Object *scene_, size_t sensor_i, uint32_t spp, fs::path filename) {
    ...
    integrator->render(scene, (uint32_t) sensor_i, 0 /* seed */, spp /* spp */, false /* develop */,
                       true /* evaluate */);
...
int render(int argc, char *argv[]) {
    ...
    auto arg_spp      = parser.add(StringVec{ "-n", "--nspp" }, true);
    ...
    try {
        ...
        mitsuba::LogLevel log_level_mitsuba[] = { Info, Debug, Trace };

        logger->set_log_level(log_level_mitsuba[std::min(log_level, 2)]);

#if defined(MI_ENABLE_CUDA) || defined(MI_ENABLE_LLVM)
        ::LogLevel log_level_drjit[] = { ::LogLevel::Error,   ::LogLevel::Warn,  ::LogLevel::Info,
                                         ::LogLevel::InfoSym, ::LogLevel::Debug, ::LogLevel::Trace };
        ...
        if (cuda || llvm) {
            ...
            if (*arg_vec_width && llvm) {
                ...
                std::string target_cpu = jit_llvm_target_cpu(), target_features = jit_llvm_target_features();

                jit_llvm_set_target(target_cpu.c_str(), target_features.c_str(), (uint32_t) width);
            }
        }
#else
...
#endif
        if (!cuda && !llvm && (*arg_optim_lev || *arg_wavefront || *arg_source || *arg_vec_width))
        ...
        size_t spp      = (*arg_spp ? arg_spp->as_int() : 0);
        ...
        while (arg_extra && *arg_extra) {
            ...
            std::vector<ref<Object>> parsed = xml::load_file(arg_extra->as_string(), mode, params, *arg_update, true);

            if (parsed.size() != 1)
                Throw("Root element of the input file is expanded into "
                      "multiple objects, only a single object is expected!");

            MI_INVOKE_VARIANT(mode, render_internal, parsed[0].get(), sensor_i, spp, filename);
